...
def execute_folder(directory_path, input_file_type, db_session, input_schema=None):
    files = [file for file in os.listdir(directory_path) if file.endswith(input_file_type.value)] # edited InputTypes in lookups
    sorted_files =  sorted(files)    
    if input_file_type == InputTypes.SQL:
        for sql_file in sorted_files:
            with open(os.path.join(directory_path,sql_file), 'r') as sql_file:
        ...
        for csv_file in sorted_files:
            csv_df = return_data_as_df(file_executor=os.path.join(directory_path,csv_file), input_type=InputTypes.CSV)
            ...
            csv_query = return_create_statement_from_df(csv_df,schema_name=input_schema,table_name=table_name)
